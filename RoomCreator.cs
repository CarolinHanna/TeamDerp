...
public class RoomCreator : MonoBehaviour {
	...
	void create_rooms(){
		foreach(Room room in rooms){
			Room new_room =  (Room)Instantiate(room_prefabs[Random.Range(0,room_prefabs.Length)], room.coord, Quaternion.identity);
			new_room.coord = room.coord;
			new_room.back = room.back;
			new_room.front = room.front;
			new_room.left = room.left;
			new_room.right = room.right;
